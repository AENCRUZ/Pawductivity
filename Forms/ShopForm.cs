...
public class ShopForm : Form
{
    ...
    private readonly AnimationPanel? _animPanel;
    private Label _lblCoins = null!;          // Label showing player's coins

    public ShopForm(GameManager gm, Action onBuy, AnimationPanel? animPanel = null)
    {
        _gm = gm;
        _onBuy = onBuy;
    ...
    private Panel BuildItemCard(ShopItem item, int y)
    {
        ...
        btnBuy.Click += (s, e) =>
        {
            if (_gm.BuyItem(item))
            {
                _animPanel?.Animator.TriggerShopItem(item.Name);
                MessageBox.Show(
